...
class Grain(object):
    ...
    def __init__(self, xy: np.ndarray, image: np.ndarray = None):
        ...
        self.selected_props = {"alpha": 1.0, "facecolor": "gold"}
        self.axes = []
        self.patch = None
        self.selected = False
    ...
    def measure(self, raster: bool = False) -> pd.Series:
        ...
        if raster:
            ...
            rasterized = rasterio.features.rasterize([poly], out_shape=image.shape[:2])
    ...
    def select(self) -> bool:
        ...
        self.patch.update(self.selected_props if self.selected else self.default_props)
        return self.selected

    # Drawing ----------------------------------------------------------------
    def draw_axes(self, ax: mpl.axes.Axes, scale: float = 1.0) -> list[mpl.artist]:
        """
    ...
    def draw_patch(
        self, ax: mpl.axes.Axes, scale: float = 1.0, animated: bool = True
    ) -> mpatches.Polygon:
        """
...
class GrainPlot(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if color_by:
            logger.info(
                f'Generating colors from "{color_palette}" colormap based on "{color_by}".'
            )
            grain_colors = self._generate_colors_by_property(
                grains, color_palette, color_by
            )
        else:
        ...
        if blit:
            self.artists = [self.info, *self.box_selector.artists]
            self.canvas.draw()
    ...
    def _generate_colors(self, n_grains: int, cmap_name: str) -> list:
        ...
        try:
            ...
            logger.warning(
                f'Unknown colormap "{cmap_name}", using "{DEFAULT_COLORMAP}"'
            )
    ...
    def _generate_colors_by_property(
        self, grains: list, cmap_name: str, property_name: str
    ) -> list:
        """
        ...
        try:
            ...
            for grain in grains:
                ...
                if property_name in grain.data:
                    ...
                    logger.warning(
                        f'Property "{property_name}" not found in grain data, using 0'
                    )
            ...
            if len(property_values) > 0 and np.ptp(property_values) > 0:
                normalized_values = (
                    property_values - np.min(property_values)
                ) / np.ptp(property_values)
            else:
            ...
            logger.info(
                f"Color range: {property_name} from {np.min(property_values):.3f} to {np.max(property_values):.3f}"
            )
            ...
            logger.warning(
                f"Error generating colors by property: {e}, using random colors"
            )
    ...
    def update(self):
        ...
        artists = (
            [g.patch for g in self.selected_grains]
            + info_patch
            + self.points
            + scale_line
            + self.artists
    ...
    def draw_axes(self):
        ...
        for grain in self.grains:
            ...
            orientation = data["orientation"]
            x_major = (
                x0 - np.sin(orientation) * 0.5 * data["major_axis_length"] * self.scale
            )
            y_major = (
                y0 - np.cos(orientation) * 0.5 * data["major_axis_length"] * self.scale
            )
            major_lines.append([(x0, y0), (x_major, y_major)])

            # Minor axis line
            x_minor = (
                x0 + np.cos(orientation) * 0.5 * data["minor_axis_length"] * self.scale
            )
            y_minor = (
                y0 - np.sin(orientation) * 0.5 * data["minor_axis_length"] * self.scale
            )
    ...
    def update_info(self, grain: Grain = None):
        ...
        if grain is None or (grain is old_grain and grain is self.info_grain_candidate):
            self.info_grain = None
            self.info_grain_candidate = None
        ...
        if self.px_per_m == 1.0:
            data = grain.data
            text = (
                f"Major: {data['major_axis_length']:.0f} px\n"
            ...
            text = (
                f"Major: {data['major_axis_length']:.3} m\n"
    ...
    def set_point(
        self, xy: tuple[int, int], foreground: bool = True
    ) -> mpatches.Circle:
        """
        ...
        """
        new_point = mpatches.Circle(
            xy, radius=5, color="lime" if foreground else "red", animated=self.blit
        )
    ...
    def create_grain(self) -> Grain:
        ...
        if len(self.points):
            points = [np.asarray(p.get_center()) / self.scale for p in self.points]
        ...
        coords = predict_from_prompts(
            predictor=self.predictor, box=box, points=points, point_labels=point_labels
        )
    ...
    def delete_grains(self):
        ...
        if len(self.selected_grains) < 1:
            ...
            if self.box_selector._selection_completed:
                xmin, xmax, ymin, ymax = (
                    np.asarray(self.box_selector.extents) / self.scale
                )
    ...
    def merge_grains(self) -> Grain:
        ...
        poly = shapely.unary_union([g.polygon for g in self.selected_grains])
    ...
    def onclickup(self, event: mpl.backend_bases.MouseEvent):
        ...
        is_info_click = (event.button == 2) or (
            event.button == 1 and event.key == "shift"
        )

        if not is_info_click or self.last_pick != (
            round(event.xdata),
            round(event.ydata),
        ):
            return
...
def polygons_to_grains(polygons: list, image: np.ndarray = None) -> list:
    ...
    for p in polygons:
        ...
        if len(p.exterior.coords) < 3:
            logger.warning(
                f"Skipping polygon with insufficient points: {len(p.exterior.coords)}"
            )
            continue
        try:
...
def get_summary(grains: list, px_per_m: float = 1.0) -> pd.DataFrame:
    """
...
def save_summary(fn: str, grains: list, px_per_m: float = 1.0):
    """
...
def get_histogram(
    grains: list = [], px_per_m: float = 1.0, summary: pd.DataFrame = None
) -> tuple[object, object]:
    """
    ...
    ret = segmenteverygrain.plot_histogram_of_axis_lengths(
        summary["major_axis_length"] * 1000, summary["minor_axis_length"] * 1000
    )
    return ret


def save_histogram(
    fn: str, grains: list = [], px_per_m: float = 1.0, summary: pd.DataFrame = None
) -> None:
    """
...
def get_mask(grains: list, image: np.ndarray) -> np.ndarray:
    ...
    rasterized_image, mask = segmenteverygrain.create_labeled_image(polys, image)
...
def filter_grains_by_points(
    grains: list, points: list, unique: False
) -> tuple[list, list]:
    """
...
def measure_color(image: np.ndarray, polygon: shapely.Polygon) -> dict:
    ...
    if xmax <= xmin or ymax <= ymin:
        logger.warning(
            f"Polygon bounds are outside image dimensions or invalid: bounds=({xmin},{ymin},{xmax},{ymax}), image=({img_height},{img_width})"
        )
    ...
    mask = rasterio.features.rasterize(
        [(clipped_polygon, 1)],
        out_shape=(h, w),
        transform=rasterio.transform.from_bounds(xmin, ymax, xmax, ymin, w, h),
        dtype=np.uint8,
    )
...
def measure_polygon(polygon: shapely.Polygon) -> dict:
    ...
    Ixy_origin = (
        np.sum((x * y_next + 2 * x * y + 2 * x_next * y_next + x_next * y) * common)
        / 24
    )
